...
public class AdminIndividual extends JFrame {
	...
	private void initComponents() {
		...
		jPanel1.setBackground(new Color(210, 202, 155));
		...
		jPanel2.setBackground(new Color(255, 0, 204));
		...
		jButton6.setText("Images");//FOR IMAGES
		...
		jLabel15.setText("Subject Student");
	...
	private void jButton6ActionPerformed(ActionEvent evt) 
	{
		...
		try {
			...
			//ImageIcon newImageIcon = new ImageIcon(person_image);
		    //lbl_img1.setIcon(newImageIcon);
		}
		...
		try
		{
			...
			String sql1="update infoTable Set image = ? where student_id= '"+value0+"'";
			pstm=con.prepareStatement(sql1);
			pstm.setBytes(1, image);
			pstm.execute();
			JOptionPane.showMessageDialog(null, "Image Successfully Updated");
		}
		catch (Exception e) {
			JOptionPane.showMessageDialog(null, e);
		} 
		...
		if (jTextField1.getText().equals("")) {
			...
			if (p == 0) 
			{
				try {
					...
					String value13 = jTextField14.getText();////TEMPORARY ADDRESS
					...
					pstm = con.prepareStatement(sql);
					//pstm.setBytes(1,person_image);
					//pstm.setString(2, value0);
					//pstm.setBytes(1,person_image);
					pstm.execute();
